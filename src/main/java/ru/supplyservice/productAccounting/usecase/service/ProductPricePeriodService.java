...
public class ProductPricePeriodService {
  ...
  @Autowired
  public ProductPricePeriodService(
      SupplierService supplierService,
      ProductService productService,
      ProductPricePeriodRepository productPricePeriodRepository,
      DTOMapper mapper) {
  ...
  public ProductPricePeriodDTO getActivePricePeriod(
      Product product, Supplier supplier, Instant date) {
    List<ProductPricePeriod> productPricePeriod =
        productPricePeriodRepository.findActivePricePeriod(product, supplier, date);
    if (productPricePeriod.isEmpty()) {
      throw new NoActivePriceFoundException(
          "Не найдена цена для "
              + product.getName()
              + " поставщика "
              + supplier.getName()
  ...
  public ProductPricePeriodDTO saveProductPricePeriod(ProductPricePeriodDTO productPricePeriodDTO) {
    Product product =
        mapper.productDTOToProduct(
            productService.getProductByName(productPricePeriodDTO.productName()));
    Supplier supplier =
        mapper.supplierDTOToSupplier(
            supplierService.getSupplierByName(productPricePeriodDTO.supplierName()));
    ProductPricePeriod productPricePeriod =
        productPricePeriodRepository.save(
            new ProductPricePeriod(
                supplier,
                product,
                productPricePeriodDTO.startDate(),
                productPricePeriodDTO.endDate(),
                productPricePeriodDTO.pricePerMeasurementUnit()));
    return mapper.productPricePeriodToProductPricePeriodDTO(productPricePeriod);
  }

  @Transactional(readOnly = true)
  public List<ProductPricePeriodDTO> getAll() {
    return productPricePeriodRepository.findAll().stream()
        .map(mapper::productPricePeriodToProductPricePeriodDTO)
        .toList();
  }
}
