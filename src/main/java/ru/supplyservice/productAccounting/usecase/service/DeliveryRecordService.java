...
public class DeliveryRecordService {
    ...
    @Autowired
    public DeliveryRecordService(SupplierService supplierService, ProductRepository productRepository, ProductPricePeriodRepository priceRepository, DeliveryRecordRepository deliveryRecordRepository, DeliveryItemRepository deliveryItemRepository, DTOMapper mapper) {
    ...
    public void saveDeliveryRecord(DeliveryRecordInfo info) {
        ...
        Set<String> productNames = info.items().stream()
                .map(ItemInfo::productName)
                .collect(Collectors.toSet());
        Map<String, Product> productMap = productRepository.findByNameIn(productNames).stream()
        ...
        Map<Product, BigDecimal> priceMap = priceRepository.findAllActivePrices(supplier, info.date()).stream()
                .collect(Collectors.toMap(ProductPricePeriod::getProduct, ProductPricePeriod::getPricePerMeasurementUnit));
        ...
        for (ItemInfo itemDTO : info.items()) {
            ...
            itemsToSave.add(new DeliveryItem(
                    product,
                    record,
                    itemDTO.quantity(),
                    itemDTO.acceptance(),
