...
class DeliveryRecordServiceTest {
    ...
    void saveDeliveryRecord_Success() {
        ...
        DeliveryRecordInfo request = new DeliveryRecordInfo(supplierName, now, "Info", List.of(itemInfo));
        ...
        when(deliveryRecordRepository.save(any(DeliveryRecord.class))).thenAnswer(i -> i.getArguments()[0]);
    ...
    void saveDeliveryRecord_ProductMissing() {
        DeliveryRecordInfo request = new DeliveryRecordInfo("Sup", Instant.now(), "Inf", 
    ...
    void saveDeliveryRecord_NoPriceForAccepted() {
        ...
        when(supplierService.getSupplierByName(any())).thenReturn(new SupplierDTO(UUID.randomUUID(), "Sup", "A"));
        ...
        NoActivePriceFoundException ex = assertThrows(NoActivePriceFoundException.class, 
                () -> service.saveDeliveryRecord(request));
    ...
    void saveDeliveryRecord_UnacceptedItem_NoPriceNeeded() {
        ...
        DeliveryRecordInfo request = new DeliveryRecordInfo("Sup", Instant.now(), "Inf", List.of(itemInfo));
        Product product = new Product();
        product.setName(prodName);
